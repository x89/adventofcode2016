class Keypad
  ...
  def U
    return if [1, 2, 3].include? @pos
    keypad_pos = keypad_idx_from_pos
    @pos = @keypad[keypad_pos[0] - 1][keypad_pos[1]]
  end

  def D
    return if [7, 8, 9].include? @pos
    keypad_pos = keypad_idx_from_pos
    @pos = @keypad[keypad_pos[0] + 1][keypad_pos[1]]
  end

  def R
    return if [3, 6, 9].include? @pos
    keypad_pos = keypad_idx_from_pos
    @pos = @keypad[keypad_pos[0]][keypad_pos[1] + 1]
  end

  def L
    return if [1, 4, 7].include? @pos
    keypad_pos = keypad_idx_from_pos
    @pos = @keypad[keypad_pos[0]][keypad_pos[1] - 1]
  end
end
...
"    ".split(' ').each do |row|
  # puts "Doing row #{row}"
  row.chars.each do |c|
    # puts "Moving #{c} from #{k.pos}"
    k.send(c)
  end
  puts "Final pos: #{k.pos}"
end
